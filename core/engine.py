...
class HybridEngine:
    ...
    async def run(self):
        ...
        try:
            ...
            # Если какая-то задача завершилась —
            # отменяем остальные
            for task in pending:
                task.cancel()
    ...
    def _on_trade(self, trade: TradeData):
        ...
        """
        # TODO Phase 2: передать в анализаторы
        # signals = []
        ...
        #     if final:
        #         await self.position_manager.open_position(final)
        
        pass  # Фаза 1: только инфраструктура
    
    def _on_orderbook_update(self, message: dict):
        """
        Callback для обновлений стакана.
        OrderBookManager уже обновлён в DataFeed.
        Фаза 2: здесь будет Depth Shot анализатор.
        """
        pass
    ...
    async def _stats_reporter(self):
        ...
        while self._running:
            ...
            logger.info(
                f"=== STATUS === "
                f"Trades: {stats['data_feed']['trade_count']} | "
                f"OB updates: "
                f"{stats['data_feed']['orderbook_updates']} | "
                f"Latency: "
                f"{stats['latency']['ws_latency_ms']}ms "
                f"({stats['latency']['current_level']}) | "
                f"API remaining: "
                f"{stats['executor']['rate_limiter']['remaining']}"
            )
    ...
    async def _cleanup(self):
        ...
        await self.alerts.send(
            ...
            f"{self.data_feed.get_stats()['trade_count']}"
        )
